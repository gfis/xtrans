...
<!-- Transformation of the root element -->
    ...
    <xsl:template match="rec:record">
        ...
        <xsl:value-of select="concat(' *  Automatically generated by xtrans/web/xslt/genRecord.xsl at ', date:date-time(), '&#10;')" />
        ...
        <xsl:variable name="spaces">
            <xsl:choose>
                <xsl:when test="string-length($FIELDNAME) &lt; 16" >
                    <xsl:value-of select="substring($spaces, 1, 16 - 2 * string-length($FIELDNAME))" />
                </xsl:when>
                <xsl:when test="string-length($FIELDNAME) &lt; 32">
                    <xsl:value-of select="substring($spaces, 1, 32 - 2 * string-length($FIELDNAME))" />
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="'                attrs.addAttribute('" />
        <xsl:value-of select="concat('&quot;&quot;, &quot;', @name, '&quot;, &quot;', @name, '&quot;', $spaces)" />
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="/rec:record/@type = 'Bean' and @type = 'double'">
                        <xsl:value-of select="concat('Double.parseDouble(', $attr, ')')" />
                    </xsl:when>
                    <xsl:when test="/rec:record/@type = 'Bean' and @type = 'float'">
                        <xsl:value-of select="concat('Float.parseFloat(', $attr, ')')" />
                    </xsl:when>
                    <xsl:when test="/rec:record/@type = 'Bean' and @type != 'String' and @type != 'string'">
