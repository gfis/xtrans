...
<!--
    ...
    @(#) $Id: gen2Record.xsl 9 2008-09-05 05:21:15Z gfis $
    2017-05-28: javadoc 1.8
    2013-12-29: remove all tab characters
    2008-08-08: generate almost no attributes
...
<!-- Transformation of the root element -->
    ...
    <xsl:template match="rec:record">
        ...
        <xsl:value-of select="concat(' *  Automatically generated by etc/xslt/gen2Record.xsl at ', date:date-time(), '&#10;')" />
    ...
    protected String replaceAttrText(String value) {
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:value-of select="concat('        generator.fireStartElement(&quot;', @name, '&quot;);', '&#10;')" />
                <xsl:for-each select="rec:array|rec:choice|rec:field">
                    <xsl:choose>
                        ...
                        <xsl:when test="name() = 'choice'">
                            <xsl:call-template name="choiceXML">
                                <xsl:with-param name="index" />
                            </xsl:call-template>
                        </xsl:when>
                        <xsl:otherwise>
...
<!-- ======== templates for sub-elements ===================== -->
    ...
    <!-- Generate set and get methods for each field, using methods of Char/ByteRecord -->
...
<!-- ======== XML generator templates ===================== -->
    <xsl:template name="arrayXML">
        ...
        <xsl:value-of select="concat('                generator.fireStartElement(&quot;', @name, '&quot;);', '&#10;')" />
        <xsl:value-of select="concat('                generator.fireSimpleElement(&quot;index&quot;, String.valueOf(', $index, '));', '&#10;')" />
        <xsl:for-each select="rec:array|rec:choice|rec:field">
            <xsl:choose>
                <xsl:when test="name() = 'array'">
                    <xsl:call-template name="arrayXML">
    ...
    <xsl:template name="choiceXML">
        <xsl:value-of select="concat('        if (false) {', '&#10;')" />
        <xsl:for-each select="rec:variant">
            <xsl:call-template name="variantXML">
            </xsl:call-template>
        </xsl:for-each>
        <xsl:value-of select="concat('        }', '&#10;')" />
    </xsl:template><!-- choiceXML -->

    <xsl:template name="variantXML">
    ...
    <xsl:template name="fieldXML">
        ...
        <xsl:value-of select="concat('                generator.fireSimpleElement(&quot;', @name, '&quot;, ')" />
        <xsl:choose>
            <xsl:when test="(@type = 'byte' and /rec:record/@type = 'Char') or @type = 'num' or @type = 'binary'">
